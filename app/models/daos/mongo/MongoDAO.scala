...
import utils.exceptions.MongoException
...
trait MongoDAO {
  ...
  def onSuccess[T](result: Future[WriteResult], entity: T): Future[T] = result.recoverWith {
    case e => Future.failed(new MongoException(s"Got exception from MongoDB  cause$e", e.getCause))
  }.map { r =>
    r.inError match {
      case true => throw new MongoException(r.message, new Throwable(new Exception(r.message)))
      case false => entity
    }
  }
  ...
  def updated(result: Future[WriteResult]): Future[Int] = result.recoverWith {
    ...
    r.inError match {
      case true => throw new MongoException(r.message, new Exception(r.message))
      case false => r.n
    }
  }
}
